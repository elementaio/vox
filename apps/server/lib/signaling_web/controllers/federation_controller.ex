defmodule SignalingWeb.FederationController do
  @moduledoc """
  ...
  alias Signaling.Federation.Policy

  @ts_window_s 300

  # GET /federation/identity — this relay's public fingerprint.
  def identity(conn, _params) do
    if Signaling.Federation.closed?() do
      forbidden(conn)
    else
      json(conn, %{relay_pubkey: Signaling.RelayIdentity.pubkey_hex()})
    end
  end
  ...
  def push(conn, %{"to" => to, "envelope" => envelope} = params) do
    cond do
      Signaling.Federation.closed?() ->
        forbidden(conn)

      accept?(conn, to, params) ->
        Signaling.Delivery.deliver(to, params["from"] || "", envelope, params["id"], params["ephemeral"] || false)
        json(conn, %{ok: true})
  ...
  defp accept?(conn, to, params) do
    ...
    is_binary(pub) and is_binary(ts) and is_binary(sig) and fresh?(ts) and
      Signaling.RelayIdentity.verify(pub, "#{pub}|#{ts}|#{to}|#{mid}", sig) and
